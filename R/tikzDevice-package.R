...
#' functions in the tikzDevice package will prefer. Current possible values are
#' \code{pdftex}, \code{xetex} or \code{luatex}. Respectively, these values
#' trigger the use of the \code{pdflatex}, \code{xelatex} and \code{lualatex}
#' compilers.  }
#' 
...
#' the device must query the LaTeX compiler for string widths and font metrics.
#' For a normal plot, this may happen dozens or hundreds of times- hence
#' becomes unresponsive for a while. The good news is that the
...
#' As you see below, the font encoding is set to Type 1. This is very important
#' so that character codes of LaTeX and match up. The default value is:
#' 
...
#' add a raster to a graphic, the raster is written to a PNG file which is then
#' included into the LaTeX code. This option controls the resolution (dpi) at
#' which the PNG files are created.  }
#' 
