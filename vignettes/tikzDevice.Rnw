...
written to a PNG file which is then included by the \LaTeX\ code. This option
controls the resolution (dpi) at which the PNG files are created.
When this options is set to \code{NA} the original raster is written
directly to a PNG file and isn't resampled.
